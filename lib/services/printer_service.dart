  ...
  Future<void> printInvoice(String content, String title, String ip) async {
    try {
      ...
      if (e.toString().contains('Connection timed out')) {
        Logger.log('❌ Error de conexión: No se pudo conectar a la impresora $ip');
  ...
  String formatInvoiceTable(String tableContent) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('-') || 
        ...
        if (columns.length >= 4) {
          String formattedRow = _formatTableRow(columns, [col1Width, col2Width, col3Width, col4Width]);
...
String processWebContent(String rawContent) {
  ...
  Logger.log('   - Después de normalizar escapes: ${processedContent.length} caracteres');
  ...
  for (int i = 0; i < lines.length; i++) {
    ...
    if (isFirstLine) {
      Logger.log('   - Línea ${i + 1}: [TÍTULO] "${line.substring(0, line.length > 50 ? 50 : line.length)}${line.length > 50 ? '...' : ''}" - OMITIDA');
      isFirstLine = false;
      continue;
    }
    ...
    if (line.startsWith('<imagen_grande>')) {
      processedLines.add(line); // Mantener la línea original para procesamiento posterior
    ...
    if (line.isNotEmpty) {
      ...
      Logger.log('   - Línea ${i + 1}: ${isCentered ? '[CENTRADA] ' : ''}${isBold ? '[NEGRITA] ' : ''}"${line.substring(0, line.length > 30 ? 30 : line.length)}${line.length > 30 ? '...' : ''}"');
    }
  }

  final result = processedLines.join('\n');
  Logger.log('   - Resultado final: ${result.length} caracteres, ${processedLines.length} líneas');
  return result;
}
    ...
    if (line.startsWith('<imagen_grande>')) {
      ...
      if (qrData.isNotEmpty) {
        try {
          ...
          Logger.log('   - 🔍 QR data problemática: ${qrData.substring(0, qrData.length > 100 ? 100 : qrData.length)}');
          lines.add(
